...
def check(sessionid: str):
    headers = {
        ...
        "User-Agent": "Mozilla/5.0 (Linux; Android 11; Pixel 4 XL Build/RQ3A.210705.001; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/83.0.4103.106 Mobile Safari/537.36 Al"
        + "iApp(Din"
        + "gTa"
        + "lk/6.5.45) com.al"
        + "iba"
    ...
    for i in range(3):
        try:
            ...
            localPrint(res.status_code, res.text)
            if res.status_code == 200:
                return True
            elif res.status_code == 400 and "已经打卡" in res.text:
                print("Already checked in", flush=True)
                return True
            else:
                print(f"#{i + 1} Failed", flush=True)
...
def getSessionId(username: str, password: str):
    print("Getting session ID", flush=True)
    options = Options()
    ...
    try:
        ...
        wait.until(EC.presence_of_element_located((By.ID, "index_log" + "in_btn")))
        ...
        print("Cannot access ca" + "s.hdu.edu.cn", flush=True)
        localPrint(repr(e))
        return ""
    ...
    try:
        ...
        print("Warning: Cannot detect login state by element", flush=True)
        localPrint(repr(e), driver.current_url)
    ...
    if loginFailed:
        ...
        if sessionId is not None and sessionId != "":
            ...
            print("Cannot get SessionId", flush=True)
            result = ""
        driver.quit()
        return result
...
def readSessionId(driver, wait):
    driver.get(
        "https://sk" + "l.h" + "duhe" + "lp.com/pas" + "scard.html#/pas" + "scard"
    )
    wait.until(EC.presence_of_element_located((By.CLASS_NAME, "nucleic-acid")))
    print("Passcard page loaded", flush=True)
...
if __name__ == "__main__":
    ...
    getRunner()
    print("Runner:", runner, flush=True)

    if isLocal():
        from secret import *

    sessionId = getSessionId(
        environ["MY_SECRET_USERNAME"], environ["MY_SECRET_PASSWORD"]
    )
    if sessionId == "":
        sys.exit(1)
    localPrint("Session Id:", sessionId)
